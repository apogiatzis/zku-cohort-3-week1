...
// [assignment] please copy the entire modified sudoku.circom here [DONE]
...
//[assignment] include your RangeProof template here [DONE]
...
template sudoku() {
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            rangeProofs[i][j][0] = RangeProof(64); // 64 bit size if even more that what we need 
            rangeProofs[i][j][0].in <== puzzle[i][j];
            rangeProofs[i][j][0].range[0] <== 0; //[assignment] change assert() to use your created RangeProof instead [DONE]
            rangeProofs[i][j][0].range[1] <== 9; //[assignment] change assert() to use your created RangeProof instead [DONE]

            rangeProofs[i][j][1] = RangeProof(64); // 64 bit size if even more that what we need 
            rangeProofs[i][j][1].in <== solution[i][j];
            rangeProofs[i][j][1].range[0] <== 0; //[assignment] change assert() to use your created RangeProof instead [DONE]
            rangeProofs[i][j][1].range[1] <== 9; //[assignment] change assert() to use your created RangeProof instead [DONE]
